...
export class nazeebo_corpse_spiders extends BaseAbility {
    ...
    //todo: test if needed?
    OnAbilityUpgrade(upgradeAbility) {
    ...
    OnSpellStart() {
        ...
        const vDirection = (cursorPos - casterPos).Normalized();
        vDirection.z = 0;
        const vDistance = (cursorPos - casterPos).Length();
        const vVelocity = (vDirection * this.speed);
        this.particle = ParticleManager.CreateParticle(this.proj_fx, 2 /* ParticleAttachment.CUSTOMORIGIN */, this.caster);
    ...
    OnProjectileHit(target, location) {
        ...
        for (let i = 0; i < nSpiders; i++) {
            const spider = CreateUnitByName("npc_dota_broodmother_spiderling", location, true, this.caster, owner, teamNumber);
            if (spider != null) {
                spider.SetOwner(this.caster);
                spider.SetControllableByPlayer(this.caster.GetPlayerOwnerID(), false);
                spider.AddNewModifier(this.caster, this, modifier_spiderling_expire.name, { duration: this.spider_duration });
