...
export function IsReflectedAbility(ability) {
    return ability.spell_shield_reflect !== undefined;
}
// Add the Reflect Ability tag to an existing ability
export function MakeReflectAbility(ability) {
    ability.spell_shield_reflect = true;
    return ability;
}
export function SpellReflect(event, parent, passive_modifier_name) {
    // List of unreflectable abilities
    const exceptionAbilities = [
        "rubick_spell_steal",
        "dark_seer_ion_shell",
        "morphling_morph",
    ...
    if (original_caster.GetTeamNumber() == parent.GetTeamNumber())
        return false;
    // Do not reflect back at reflectors to prevent infinite loops
    if (original_caster.HasModifier("modifier_item_lotus_orb_active") ||
    ...
    if (exceptionAbilities.includes(event.ability.GetAbilityName()))
        return false;
    // Do not reflect abilities that have the reflect tag
    if (IsReflectedAbility(event.ability))
        return false;
    ...
    if (parent.HasModifier(passive_modifier_name)) {
        ...
        if (passive_modifier) {
            passive_modifier.reflected_abilities.push(reflected_ability_handle);
        }
    }
    return true;
}
export function RemoveReflectedAbilities(modifier) {
    let removeable_abilities = [];
...
export function CalculateDistanceBetweenEntities(entity1, entity2) {
    let distance = (entity1.GetAbsOrigin() - entity2.GetAbsOrigin()).Length2D();
    return distance;
}
export function CalculateDistanceBetweenPoints(point1, point2) {
    let distance = (point1 - point2).Length2D();
    return distance;
}
export function CalculateDirectionToPosition(origin_pos, towards_pos) {
    return (towards_pos - origin_pos).Normalized();
}
/**
 ...
 */
export function FindUnitsAroundUnit(caster, around_unit, radius, findTeam, findtypes, findFlags) {
    let units = [];
    units = FindUnitsInRadius(caster.GetTeamNumber(), around_unit.GetAbsOrigin(), undefined, radius, findTeam, findtypes, findFlags, 0 /* FindOrder.ANY */, false);
    return units;
}
export function FindUnitsInRadiusOuterRing(caster, position, radius, findTeam, findtypes, findFlags, min_distance) {
    let units = [];
    units = FindUnitsInRadius(caster.GetTeamNumber(), position, undefined, radius, findTeam, findtypes, findFlags, 0 /* FindOrder.ANY */, false);
...
export function CanUnitGetOrders(unit) {
...
export function FindUnitsInCone(teamNumber, vDirection, vPosition, startRadius, endRadius, flLength, hCacheUnit, targetTeam, targetUnit, targetFlags, findOrder, bCache) {
    ...
    if (enemies.length > 0) {
        for (const enemy of enemies) {
            if (enemy != undefined) {
                const vToPotentialTarget = (enemy.GetAbsOrigin() - vPosition);
...
export function FindBiggestAbilityTargetCluster(ability, at_location, initial_search_radius, cluster_radius, bigger_than) {
...
export function FindBiggestCluster(all_clusters, bigger_than) {
    let biggest_cluster = undefined;
...
export function SplitAbilityTargetsIntoCircleClusters(ability, at_location, initial_search_radius, cluster_radius) {
...
export function ClusterizePoints(points, cluster_radius) {
    let all_clusters = [];
    for (let index_top = 0; index_top < points.length; index_top++) {
        ...
        for (let index_bottom = 0; index_bottom < points.length; index_bottom++) {
            ...
            if (index_top != index_bottom && (point_bottom - point_top).Length2D() <= cluster_radius * 2) {
...
export function FindPredictedAbilityTargetsPointsAtLocation(ability, at_location, initial_search_radius) {
...
export function FindAbilityTargetsAtLocation(ability, at_location, with_radius) {
    const units = FindUnitsInRadius(ability.GetMoveParent().GetTeamNumber(), at_location, undefined, with_radius, ability.GetAbilityTargetTeam(), ability.GetAbilityTargetType(), ability.GetAbilityTargetFlags(), 0 /* FindOrder.ANY */, false);
...
export function CustomCleaveAttack(attacker, start_position, start_width, end_width, distance, targetTeam, targetType, targetFlags, target, damage_percent, ability) {
    // Find enemies
    const enemies = FindUnitsInCone(attacker.GetTeamNumber(), attacker.GetForwardVector(), start_position, start_width, end_width, distance, undefined, targetTeam, targetType, targetFlags, 1 /* FindOrder.CLOSEST */, false);
    // Calculate damage
    ...
    for (const enemy of enemies) {
        ...
        ApplyDamage({
            ...
            victim: enemy,
            ability: ability,
            damage_flags: 0 /* DamageFlag.NONE */,
        });
    }
    return enemies.length;
}
export function PerformAttackNoCleave(attacker, target, useCastAttackOrb, processProcs, skipCooldown, ignoreInvis, useProjectile, fakeAttack, neverMiss) {
    ...
    if (fakeAttack) {
        ...
        const modifier = attacker.AddNewModifier(undefined, undefined, "modifier_reimagined_no_outgoing_damage" /* GenericModifier.NO_OUTGOING_DAMAGE */, { duration: 1 });
...
export function PrintEventTable(event) {
    for (const key in event) {
        print(key, event[key]);
    }
}
export function IsRoshan(unit) {
    if (unit.GetName() == "npc_dota_roshan")
        return true;
    else
        return false;
}
export function CanOrbEffectBeCast(event, ability, orb_data) {
...
export function CanUserCastOrb(user, ability, can_proc_from_illusions, can_proc_while_silenced, mana_cost) {
...
export function CanOrbBeCastOnTarget(target, can_proc_on_building, can_proc_on_wards, can_proc_on_magic_immune) {
 ...
 */
export function GetAppliedDuration(caster, target, duration, modifier_name) {
    // Does nothing if caster and target are on the same team
    if (caster && caster.GetTeamNumber() == target.GetTeamNumber())
        return duration;
    ...
    if (caster) {
        ...
        for (const modifier of modifiers) {
            status_amp.push(caster.GetStatusResistance());
            // if (modifier.GetModifierStatusResistanceCaster) {//TODO: comment? not used?
...
export function HasBit(checker, value) {
    return (checker & value) == value;
}
export function GenerateRandomPositionAroundPosition(position, min_distance, max_distance) {
    return (position + RandomVector(min_distance + Math.sqrt(RandomFloat(0, 1)) * (max_distance - min_distance)));
}
export function GenerateRandomPositionAroundPositionAngled(position, direction, min_distance, max_distance, min_angle, max_angle) {
    let random_pos = (position + direction * RandomInt(min_distance, max_distance));
    const qangle = QAngle(0, RandomInt(min_angle, max_angle), 0);
    let final_pos = RotatePosition(position, qangle, random_pos);
    return final_pos;
}
// Serverside talent utility functions
export function HasTalent(caster, name) {
    if (IsServer()) {
...
let specialValueTable = new Map();
let abilityKVValueFile = new Map();
export function GetTalentSpecialValueFor(caster, name, value) {
    let number = 0;
    if (IsServer()) {
        ...
        if (HasTalent(caster, name)) {
            ...
            if (abilityKVValueFile.has(filepath)) {
                ...
                abilitiesKV = LoadKeyValues(filepath);
                ...
                for (const key in abilitiesKV[name][abilityKVSpecial]) {
                    ...
                    for (const key2 in abilitiesKV[name][abilityKVSpecial][key]) {
                        if (key2 == value) {
                            ...
                            const actual_value = abilitiesKV[name][abilityKVSpecial][key][key2];
...
export function IsTalentAbility(ability) {
    if (ability.isTalentAbility)
        return true;
    if (ability.GetAbilityName().indexOf("special_bonus") !== -1)
        return true;
    return false;
}
export function PrepareTalentList(hero) {
...
export function GetTalentNumber(ability) {
    // Get caster
...
export function GetTalentAbilityFromNumber(hero, talent_number) {
    if (hero.talentMap.has(talent_number)) {
        return hero.talentMap.get(talent_number);
    }
    return undefined;
}
export function ShuffleNumbersInArray(number_array) {
...
export function IsSpiderlingUnit(unit, includes_spiderking) {
    if ((includes_spiderking && unit.GetUnitName() === "npc_dota_reimagined_broodmother_spiderking") || IsSpiderling(unit) || unit.GetUnitName() === "npc_dota_broodmother_spiderite")
        return true;
    else
        return false;
}
export function IsSpiderling(unit) {
    if (unit.GetUnitName() === "npc_dota_broodmother_spiderling")
        return true;
    else
        return false;
}
export function IsNearEntity(entity_name, location, distance, owner) {
...
export function GetChargeModifierForAbility(ability) {
    // Find all charge modifiers
    const caster = ability.GetCaster();
    const modifiers = caster.FindAllModifiersByName("modifier_reimagined_charges" /* GenericModifier.CHARGES */);
    for (const modifier of modifiers) {
...
export function GetAllChargesModifiersForUnit(caster) {
    const charge_modifiers = caster.FindAllModifiersByName("modifier_reimagined_charges");
...
export function AddNewModifier(caster, ability, modifierName, modifierTable) {
...
export function GetIllusionOwner(illusion_unit) {
    if (!illusion_unit.IsIllusion())
        return undefined;
    let possible_owners = Entities.FindAllByName(illusion_unit.GetUnitName());
...
export function GetIllusions(unit) {
    if (unit.IsIllusion())
        return undefined;
    let possible_illusions = Entities.FindAllByName(unit.GetUnitName());
...
export function ConvertRadiansToEffectiveDotRange(radians) {
    const effectiveDot = math.cos(radians);
    return effectiveDot;
}
// Rotate a 2D Vector clockwise by x degrees
export function RotateVector2D(direction, degree) {
    let theta = ConvertDegreesToRadians(degree);
    let xp = direction.x * Math.cos(theta) - direction.y * Math.sin(theta);
    let yp = direction.x * Math.sin(theta) + direction.y * Math.cos(theta);
    return Vector(xp, yp, direction.z).Normalized();
}
// Calculate the angle between two vectors as degrees (with sign as side indicator)
export function AngleCalculationWithSide(vectorA, vectorB) {
...
export function GetVectorSide(direction, origin, point) {
    let goal = (origin + direction * 1000);
...
export function MinimumDistanceToLine(startLoc, endLoc, testLoc) {
    let l2 = sqrDist(startLoc, endLoc);
    if (l2 == 0)
        return sqrDist(startLoc, testLoc);
...
function sqrDist(vectorA, vectorB) {
...
export function ConvertDegreesToRadians(deg) {
    return deg * (Math.PI / 180);
}
export function ConvertRadiansToDegrees(rad) {
    return (360 * rad) / (Math.PI * 2);
}
export function HasScepterShard(unit) {
    return unit.HasModifier("modifier_item_aghanims_shard");
}
export function SpawnDummyUnit(location, owner) {
    return CreateUnitByName("reimagined_npc_dummy_unit", location, false, owner, owner, owner.GetTeamNumber());
}
export function IsInRiver(unit) {
    if (unit.GetAbsOrigin().z <= 15)
        return true;
    if (unit.HasModifier("modifier_reimagined_slardar_puddle"))
        return true;
    return false;
}
export function GetAttackDotProduct(attacker, target) {
...
export function IsTeleporting(unit) {
    return unit.HasModifier("modifier_teleporting");
}
export function AddActiveItemModifierArray(item) {
    const item_name = item.GetName();
    const wielder = item.GetCaster();
    if (!wielder.active_item_modifiers)
        wielder.active_item_modifiers = new Map();
...
export function IsActiveModifierItem(item) {
    const item_name = item.GetName();
    const wielder = item.GetCaster();
    if (!wielder.active_item_modifiers)
        return false;
...
export function RemoveActiveModifierItem(item) {
    const item_name = item.GetName();
    const wielder = item.GetCaster();
    if (!wielder.active_item_modifiers || !wielder.active_item_modifiers.has(item_name))
        return;
    const item_modifier_array = wielder.active_item_modifiers.get(item_name);
    if (item_modifier_array) {
...
export function IsStunModifier(modifier_name, include_bashes) {
    if (include_bashes && modifier_name == "modifier_bashed")
        return true;
    if (modifier_name.indexOf("_stun") != -1)
        return true;
    ...
    if (stun_modifier_names.includes(modifier_name))
        return true;
    return false;
}
export function GenerateRandomConsumableItem(rare_consumables_chance) {
    const consumables = [
        "item_faerie_fire",
...
export function GenerateRandomArrayElement(array) {
    return array[RandomInt(0, array.length - 1)];
}
export function CreateBagOfGoldInPosition(gold_min, gold_max, position, lifetime) {
    const gold_bag = CreateItem("item_bag_of_gold", undefined, undefined);
    if (gold_bag != null) {
...
export function CreateItemInPosition(item_name, position, lifetime) {
