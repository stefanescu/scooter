...
export function GetChargeModifierForAbility(ability: CDOTA_Ability_Lua): modifier_charges | undefined {
	// Find all charge modifiers
	const caster = ability.GetCaster();
	const modifiers = caster.FindAllModifiersByName(GenericModifier.CHARGES) as modifier_charges[];
	for (const modifier of modifiers) {
